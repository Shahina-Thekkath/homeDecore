...
const getAddOffer = async (req, res) => {
  try {
    ...
    logger.error(("Error loading add offer page:", error));
    res
...
const getOfferList = async (req, res) => {
  try {
    ...
    logger.error(("Error fetching offer list:", error));
    res
      .status(STATUS_CODES.INTERNAL_SERVER_ERROR)
  ...
  try {
    ...
    if (!offer) {
      return res
        ...
        .json({ message: MESSAGES.OFFER.NOT_FOUND });
    }
    ...
    res
      ...
      .json({
        message: `${offerType === "product" ? "Product" : "Category"} offer ${
    ...
    res
      .status(STATUS_CODES.INTERNAL_SERVER_ERROR)
      .json({ message: MESSAGES.GENERIC.INTERNAL_ERROR });
  }
};
...
const getEditOffer = async (req, res) => {
  try {
    ...
    if (!offer)
      return res.status(STATUS_CODES.NOT_FOUND).send(MESSAGES.OFFER.NOT_FOUND);
...
const updateOffer = async (req, res) => {
  try {
    ...
    if (offerType === "product") {
      ...
      if (!updated) {
        return res
          ...
          .json({
            success: false,
            message: MESSAGES.OFFER.PRODUCT_OFFER_NOT_FOUND,
          });
      }

      // unlink from old product if product changes
      if (oldOffer && oldOffer.productId.toString() !== productId) {
        await Product.findByIdAndUpdate(oldOffer.productId, {
          $unset: { productOffer: 1 },
        });
      }
      ...
      if (oldOffer && oldOffer.categoryId.toString() !== categoryId) {
        await Category.findByIdAndUpdate(oldOffer.categoryId, {
          $unset: { categoryOffer: 1 },
        });
      }
    ...
    res
      .status(STATUS_CODES.OK)
      .json({ success: true, message: MESSAGES.OFFER.UPDATED_SUCCESS });
  } catch (error) {
