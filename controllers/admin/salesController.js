...
const getSalesReport = async (req, res) => {
  try {
    return res.render("salesReport", {
      currentPage: 1,
      totalPages: 1,
      baseUrl: "/admin/salesReport",
    });
  } catch (error) {
    logger.error("Error loading sales Report page", error);
    res
      .json(STATUS_CODES.INTERNAL_SERVER_ERROR)
      .send(MESSAGES.GENERIC.INTERNAL_ERROR);
  }
};
...
const getSalesReportData = async (req, res) => {
  try {
    ...
    const [ordersRaw, summaryArr] = await Promise.all([
      ordersPromise,
      summaryPromise,
    ]);
    ...
    logger.error("Error building sales report:", err);
    res
      .status(STATUS_CODES.INTERNAL_SERVER_ERROR)
      .json({ success: false, message: MESSAGES.GENERIC.INTERNAL_ERROR });
  }
};
  ...
  try {
    ...
    if (!orders || orders.length === 0) {
      return res
        ...
        .json({ success: false, message: MESSAGES.SALES.NO_DATA_FOUND });
    }
    ...
    const fonts = {
      Roboto: {
        normal: path.join(
          __dirname,
          "../../admin-public/assets/fonts/Noto_Serif,Roboto/Roboto/static/Roboto_SemiCondensed-Regular.ttf"
        ),
        bold: path.join(
          __dirname,
          "../../admin-public/assets/fonts/Noto_Serif,Roboto/Roboto/static/Roboto_SemiCondensed-Bold.ttf"
        ),
        italics: path.join(
          __dirname,
          "../../admin-public/assets/fonts/Noto_Serif,Roboto/Roboto/static/Roboto_Condensed-Italic.ttf"
        ),
        bolditalics: path.join(
          __dirname,
          "../../admin-public/assets/fonts/Noto_Serif,Roboto/Roboto/static/Roboto_Condensed-BlackItalic.ttf"
        ),
      },
    };
          ...
          stack: [
            {
              image: path.join(
                __dirname,
                "../../admin-public/assets/The Elegant Adobe-(Compressify.io)-1.png"
              ),
              width: 150,
              margin: [0, 0, 0, 15],
            },
          ...
          table: {
            ...
            body: [
              ...
              ...data.orders.map((o) => [
                ...
                {
                  text: `₹${o.salesAmount.toFixed(2)}`,
                  style: "tableCell",
                  alignment: "right",
                },
                {
                  text: `₹${o.discount.toFixed(2)}`,
                  style: "tableCell",
                  alignment: "right",
                },
                {
                  text: `₹${o.coupon.toFixed(2)}`,
                  style: "tableCell",
                  alignment: "right",
                },
                {
                  text: `₹${o.netSales.toFixed(2)}`,
                  style: "tableCell",
                  alignment: "right",
                  bold: true,
                },
          ...
          table: {
            ...
            body: [
              [
                ...
                {
                  text: `₹${summary.totalDiscount.toFixed(2)}`,
                  style: "summaryValue",
                  alignment: "right",
                },
              ],
              [
                ...
                {
                  text: `₹${summary.totalCoupon.toFixed(2)}`,
                  style: "summaryValue",
                  alignment: "right",
                },
              ],
              [
                ...
                {
                  text: `₹${summary.totalAmount.toFixed(2)}`,
                  style: "summaryTotal",
                  alignment: "right",
                },
              ],
...
const generateSalesExcel = async (req, res) => {
  try {
    ...
    worksheet.getCell("A1").value = `Sales Report (${
      type === "custom" ? `${startDate} to ${endDate}` : type.toUpperCase()
    })`;
    ...
    res
      .status(STATUS_CODES.INTERNAL_SERVER_ERROR)
      .send(MESSAGES.SALES.EXCEL_GENERATION_FAILED);
  }
};
