...
const addAddress = async (req, res) => {
  try {
    const userId = req.session.user?._id || req.session.passport?._id;
    const { name, address, city, state, locality, pincode, country, phone } =
      req.body;
    ...
    return res
      ...
      .json({ message: MESSAGES.ADDRESS.ADDED });
...
const loadEditAddress = async (req, res) => {
  try {
    ...
    logger.error("Error Fetching Edit Address", error);
...
const editAddress = async (req, res) => {
  try {
    ...
    const { name, locality, phone, address, country, state, city, pincode } =
      req.body;
    ...
    if (
      ...
      return res
        ...
        .json({ message: MESSAGES.VALIDATION.ALL_FIELDS_REQUIRED });
    }
...
const deleteAddress = async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(STATUS_CODES.BAD_REQUEST)
        .json({ ok: false, message: MESSAGES.USER.NOT_FOUND });
    }
