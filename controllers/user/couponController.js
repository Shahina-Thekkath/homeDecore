  ...
  try {
    ...
    if (!couponCode) {
      return res
        ...
        .json({ success: false, message: MESSAGES.COUPON.CODE_REQUIRED });
    }
    ...
    if (!coupon.isActive) {
      return res
        .status(STATUS_CODES.OK)
        .json({ success: false, message: MESSAGES.COUPON.INACTIVE });
    }

    if (new Date(coupon.expiresOn) < new Date()) {
      return res
        .status(STATUS_CODES.OK)
        .json({ success: false, message: MESSAGES.COUPON.EXPIRED });
    }

    if (coupon.usageLimit <= coupon.usedCount) {
      return res
        .status(STATUS_CODES.OK)
        .json({ success: false, message: MESSAGES.COUPON.USAGE_LIMIT_REACHED });
    }
    ...
    if (!cart || cart.items.length === 0) {
      return res
        .status(STATUS_CODES.OK)
        .json({ success: false, message: MESSAGES.COUPON.EMPTY_CART });
    }
  ...
  try {
    ...
    if (!cart || cart.items.length === 0) {
      return res
        ...
        .json({ success: false, message: MESSAGES.COUPON.CART_EMPTY });
    }
    ...
    logger.error("Error removing coupon:", err);
    return res
      .status(STATUS_CODES.INTERNAL_SERVER_ERROR)
      .json({ success: false, message: MESSAGES.GENERIC.INTERNAL_ERROR });
  }
...
export default { applyCoupon, removeCoupon };